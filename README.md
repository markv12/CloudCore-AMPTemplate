# CloudCore — AMP Configuration Repository

This repo lets anyone run the CloudCore dedicated server in [AMP](https://cubecoders.com/AMP)
with almost no setup: they add this repo once, pick **CloudCore** from the app list, and hit
...
| `cloudcore.kvp` | Module definition (executable, args, env, ready/stop behaviour). |
| `cloudcoreconfig.json` | User-editable settings: port, save name, player limit. |
| `cloudcoremetaconfig.json` | Empty — the server is arg-driven, no config file. |
| `cloudcoreports.json` | Single UDP game port (default 10000). |
| `cloudcoreupdates.json` | Update logic: download the build zip from Google Drive → unzip → chmod +x. |
| `package-server.ps1` | (In `BuildServer/`) zips the Linux build into `CloudCoreServer-Linux.zip`. |

The build zip itself is **not** committed (see `.gitignore`) — it's hosted on Google Drive.

The build zip is hosted on **Google Drive**; `cloudcoreupdates.json` downloads it via a
...
Repo: https://github.com/markv12/CloudCore-AMPTemplate — the template files are pushed to
`main`. The Drive download URL is already baked into `cloudcoreupdates.json`, so there is
...
1. **Add the repo once**: AMP → *Configuration → Instance Deployment → Add → Configuration Repository*,
   enter `markv12/CloudCore-AMPTemplate:main`.
2. **Create an instance** and choose **CloudCore** from the application list.
...
you'd have to update the URL in `cloudcoreupdates.json` and re-push.)
...
- **Launch**: `CloudCoreServer.x86_64 -batchmode -nographics -logfile /dev/stdout -bind 0.0.0.0 -port <allocated> -save <name>`
...
- **Saves**: `XDG_CONFIG_HOME`/`HOME` are pointed at `<instance>/CloudCore/saves`, so worlds
